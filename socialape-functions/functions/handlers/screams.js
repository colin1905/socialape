...
exports.getScream = (req, res) => {
  ...
  .then(data => {
    ...
    return res.json(screamData); //already JSON
  })
  .catch(err => {
    console.error(err);
    res.status(500).json({ error: err.code })
...
exports.likeScream = (req, res) => {
  //check whether a like already exists, and whether the scream itself exists
  ...
  .then(data => {
    if(data.empty){return db.collection('likes').add({
      screamId: req.params.screamId,
      userHandle: req.user.handle
    })
    .then(() => {
